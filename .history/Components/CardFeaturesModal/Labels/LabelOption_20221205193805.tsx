...
const LabelOption = ({
  ...
  const updateLabel = (colorChoices: ILabelColors[]) => {
    onUpdateData(colorChoices);
  };
  ...
  useEffect(() => {
    ...
    if (currentBgColor) {
      ...
      console.log(currentBgColor?.bgColor);
      setColorBg(color);
  ...
  return (
    <div className='z-20 absolute flex flex-col w-72 shadow-md rounded-sm bg-white px-3 py-2 gap-3'>
      ...
      <button type='button' className='bg-gray-200 text-sm py-2'>
        Create a new label
      </button>
    </div>
  );
