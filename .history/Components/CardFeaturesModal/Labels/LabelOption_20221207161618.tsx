...
const LabelOption = ({
  ...
  const [colorState, setColorState] = useState<IColorState | undefined>(color);
  ...
  const data = {
    ...
    colorBg: colorState?.bgColor,
    colorIcon: colorState?.iconColor,
    colorBgStrip: colorState?.bgColorStrip,
  };
  ...
  const createLabel = (event: React.MouseEvent<HTMLButtonElement>) => {
    // const id = event.currentTarget.dataset.id;
    // const updatedData = {
    //   id: id,
    //   data: data,
    ...
    //   payload: id,
    // });
    console.log({ ...data, name: createTitle });
  };
