...
const CardFeaturesModal = ({ _id, cardName, listName }: IBoardCardData) => {
  ...
  return (
    <div
      ...
      className='z-10 absolute top-0-left-0 xs:relative flex flex-col w-screen xs:w-auto xs:max-w-3xl py-4 px-1 sm:py-7 sm:px-5 gap-6 bg-gray-100 text-gray-700'
    >
      ...
      <div className='flex flex-row gap-6'>
        ...
        >
          <div className='relative min-h-screen sm:h-auto p-4 sm:p-0'>
            ...
            <button
              ...
              className='absolute top-1/2 right-0 translate-x-full flex sm:hidden w-6 h-10 rounded-r-full bg-gray-800 hover:bg-gray-700 text-gray-200'
              onClick={toggleSideBarHandler}
            >
              <FontAwesomeIcon
                icon={faChevronRight}
                size='lg'
                className='text-gray-300 translate-x-1.5 translate-y-2.5'
              />
            </button>
          </div>
        </section>
        <button
          ...
          } z-10 absolute top-1.5 right-1.5 rounded-full py-1.5 px-3 hover:bg-gray-800`}
