...
const CardFeaturesModal = ({ _id, cardName, listName }: IBoardCardData) => {
  ...
  return (
    <div
      ...
      className='z-10 relative flex flex-col min-w-screen xs:max-w-3xl py-4 px-1 gap-6 bg-gray-100 text-gray-700'
    >
      ...
      <div className='flex flex-row gap-6'>
        ...
        <section
          ...
          } transition xs:translate-x-0 fixed bg-white min-h-screen xs:h-auto xs:bg-transparent shadow-2xl xs:shadow-none top-0 left-0 xs:relative flex flex-col gap-4 w-44`}
        >
          <div className='relative min-h-screen xs:h-auto p-4 xs:p-0'>
            <Features _id={_id} />
            <button
              type='button'
              className='absolute top-1/2 right-0 translate-x-full w-8 h-10 rounded-r-full bg-gray-800 text-gray-200'
