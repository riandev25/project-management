...
const ChecklistModal = () => {
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    dispatchChecklist({
      type: 'ADD_CHECKLIST',
      payload: { data: checklist, idCheckArray: checklistData.length },
    });
    dispatchFeature({
      type: 'TOGGLE_FEATURE',
      payload: { id: 'checklist-option' },
    ...
    >
      ...
      <section className='flex flex-col gap-1'>
        ...
        <input
          ...
          placeholder='Title'
          onChange={(event) => setChecklist({ title: event.target.value })}
        />
        <h3 className='text-xs font-semibold text-gray-600'>Title</h3>
        <input
          type='text'
          ...
          className='w-full border-2 border-solid border-gray-200 px-2 py-1 text-sm'
          placeholder='Title'
          onChange={(event) => setChecklist({ checklist: event.target.value })}
        />
      </section>
      <SubmitBtn
        id='checklist-option'
