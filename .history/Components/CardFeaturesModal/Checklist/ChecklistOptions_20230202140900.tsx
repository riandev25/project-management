...
interface IChecklistOption {
  ...
  toggleOptionsExtension: (isOptionOpen: boolean | undefined) => void;
}
...
const ChecklistOptions = ({
  ...
  const toggleOptionsHandler = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    toggleOptionsExtension(isOptionOpen);
  };

  return (
    <div
      className={`${
        isOptionOpen ? 'flex bg-gray-300' : 'hidden'
      } group-hover:flex absolute top-1.5 right-0 pl-12 justify-end`}
    >
