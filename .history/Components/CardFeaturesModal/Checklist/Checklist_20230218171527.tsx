...
const Checklist = ({ _id, name, checkitem }: IChecklistArrays) => {
  ...
  useEffect(() => {
    const filteredCheckItem = checkitem.filter((checkitem) => {
      if (checkitem.dueDate) {
        const { dueDate } = getDateTime(checkitem.dueDate);
            ...
            {(provided) => (
              ...
              >
                {checkitemData.map((item, i) => {
                  const { dueDate, formattedDate, formattedTime } = getDateTime(item.dueDate);
                  return (
                    <Draggable
                      key={item._id}
                        ...
                        >
                          <div
                            ...
                            className={`relative group flex flex-row py-2 justify-center gap-4 ${
                              dueDate ? 'bg-red-300' : 'hover:bg-gray-200'
                            }`}
                          >
                            <input
                              data-id={item._id}
