...
const Checklist = ({ title, percentage, checklistItems }: IChecklistArray) => {
  ...
  const [percentState, setPercentState] = useState<number | null>(null);
  ...
  useEffect(() => {
    const checklistPercentage = () => {
      const percentItems = checklistItems.map((checklist) => {
        return checklist.isChecked ? checklist : null;
      });
    ...
    <div className='flex flex-col gap-4'>
      ...
      <div className='flex flex-col gap-1 text-gray-800'>
        <section className='flex flex-row justify-start items-center gap-4'>
          <h3 className='text-xs px-1'>{`${percentState}%`}</h3>
          <div
