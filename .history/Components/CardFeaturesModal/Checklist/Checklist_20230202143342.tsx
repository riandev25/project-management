...
const Checklist = ({ _id, name, checkitem }: IChecklistArrays) => {
  ...
  const [isOptionState, setIsOptionState] = useState<boolean | undefined>(
    false
  );
        ...
        <ul className='flex flex-col'>
          {checkitem.map((item, i) => {
            return (
              <li
                key={i}
                className={`relative group flex flex-row py-2 justify-center gap-4 cursor-pointer ${
                  isOptionState ? 'bg-gray-200' : ''
                } hover:bg-gray-200`}
              >
                <input
                  data-id={item._id}
