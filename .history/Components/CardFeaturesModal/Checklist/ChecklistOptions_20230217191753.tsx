...
const ChecklistOptions = ({ id }: IChecklistOption) => {
  const { checkitemState, toggleOptionState, toggleDateOptionState } = checkitemStore(
    (state) => ({
      ...
      <div className='relative flex flex-row gap-1 pl-12 justify-end'>
        ...
        {/* <div className='absolute z-20 right-0'> */}
          {isOptionOpen ? (
            ...
            />
          ): isDateOptionOpen ? <ChecklistDueDate id={id} exitBtnHandler={toggleDateHandler} /> : null}
        {/* </div> */}
      </div>
    </div>
  );
