...
const LabelsModal = () => {
  const { closeLabel } = useContext(FeatureContext);
  const { dispatchBtnLabel } = useContext(LabelContext);
  const { labelsBtnState } = useContext(LabelContext);
    ...
    <div className='absolute flex flex-col w-72 shadow-md rounded-sm bg-white px-3 py-2 gap-3'>
      ...
      <section className='space-y-2'>
        ...
        <ul className='grid grid-cols-1 gap-1.5'>
          {labelsBtnState.map(({ id, bgColor, name, iconColor, isChecked }) => {
            return (
              <li
                key={id}
