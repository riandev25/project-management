...
import { CardChildren, LabelProp, Labels } from '../../../interfaces/data';
import dataReducer from '../../../lib/context/DataContext/dataReducer';
...
const LabelsModal = ({ labels }: LabelProp) => {
  const { closeLabel } = useContext(useFeatureContext);

  const [LabelState, dispatch] = useReducer(dataReducer, labels);

  // const onChangeHandler = (event: { currentTarget: { dataset: { id: string; }; }; }) => {
