...
const ListModal = ({ toggleBtn, setToggleBtn, editor }: IListModal) => {
  ...
  return (
    <div
      className={`absolute z-30 top-8 -translate-x-28 sm:translate-x-0 sm:left-2 ${
        toggleBtn ? 'grid' : 'hidden'
      } grid-cols-1 w-[calc(100vw-100px)] xs:w-72 border border-gray-300 rounded-md shadow-md bg-white`}
    >
      {OTHER.map((item, i) => {
        return (
          <button
