...
const BoardList = () => {
  ...
  return (
    <div className='relative flex flex-col w-full xs:w-[50rem] px-3 xs:px-6 bg-gray-800 text-gray-700 font-bold'>
      <h1 className='px-2 py-3'>Your workspaces</h1>
      <ul className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
        {responseData.map((item: any, i) => {
          return (
            <li
              className='relative flex h-24 bg-gray-300 rounded-md hover:bg-gray-700 animate-pulse'
              key={i}
            >
              <Link
        ...
        <li className='relative h-24 bg-gray-300 rounded-md hover:bg-gray-400'>
          <button
