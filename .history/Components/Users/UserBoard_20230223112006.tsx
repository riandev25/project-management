...
const UserBoard = () => {
  ...
  return (
    <div className='relative flex flex-row justify-center w-full min-h-screen py-12 bg-gray-800'>
      <BoardList />
      <button
        className='absolute top-4 right-4 text-sm sm:text-base text-gray-800 px-2 py-1 xs:px-4 xs:py-2 bg-transparent border border-gray-300 transition-colors rounded-md hover:bg-gray-300'
        type='button'
      ...
      {isModalOpen ? (
        <div className='z-20 absolute top-0 left-0 w-screen h-screen bg-black opacity-70'></div>
      ) : null}
    </div>
  );
};
