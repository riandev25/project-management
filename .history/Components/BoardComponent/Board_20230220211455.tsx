...
const BoardComponent = () => {
  ...
  console.log(isOneCardOptionOpen);
  ...
  return (
    <div className='relative flex flex-col px-2 sm:px-3 gap-4 sm:gap-3 min-h-screen'>
      <BoardHeader />
      <ul className='flex flex-row items-start gap-4 overflow-x-hidden xs:overflow-x-auto xs:flex-1 w-full snap-x'>
        {listData.map(({ _id, listName, cards }: IBoardData, i: number) => {
          return (
            <li key={i} className='max-w-screen xs:w-auto snap-center'>
