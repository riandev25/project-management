...
const BoardComponent = () => {
  ...
  const [lists, setLists] = useState<IBoardData[]>([]);
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (lists !== undefined) {
      setLists((lists: any) => {
        const sourceList = lists.find((list: any) => list._id === source.droppableId);
        const destinationList = lists.find(
          (list: any) => list._id === destination.droppableId
        );
        const draggedCard = sourceList.cards.find(
          (card: any) => card._id === draggableId
        );
        ...
        // Save only the order to local storage
        setLocalStorage('lists', JSON.stringify(newLists.map((list: any) => list.cards.map((card: any) => card._id))));
        return newLists;
      });
    }
  };
