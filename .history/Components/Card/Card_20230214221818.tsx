...
const Card = ({ cards, listName, _id }: IBoardData) => {
  ...
  return (
    <div className='relative flex flex-col flex-1 w-[17rem] bg-gray-300 px-1.5 pb-2 rounded-sm text-sm text-gray-700'>
      ...
      {!isAddCardOpen && (
        <section className='flex flex-row gap-2 mt-2.5 text-gray-500'>
          <button
            data-id={_id}
            type='button'
            className='btn relative inline-flex items-center justify-start p-2 overflow-hidden transition-all group'
            // className='w-full text-start rounded-sm py-1.5 px-2 space-x-2 hover:bg-gray-400'
            onClick={toggleAddCardHandler}
          >
            <span className='w-0 h-full bg-white absolute top-0 left-0 ease-out duration-500 transition-all group-hover:w-full -z-1 space-x-2'></span>
            <FontAwesomeIcon
              icon={faAdd}
              className='group-hover:text-gray-500'
            />
            <span className='group-hover:text-gray-500'>Add a card</span>
          </button>
        </section>
      )}
