...
//   return (
//     <div className='relative flex flex-col flex-1 w-[calc(100vw-32px)] min-w-[calc(100vw-32px)] xs:min-w-0 xs:w-[17rem] bg-gray-300 px-1.5 pb-2 rounded-sm text-sm text-gray-700'>
...
const Card = ({ cards, labels, listName, _id }: IBoardData) => {
  // Global store
  const { listState, toggleAddCard, toggleCardOption } = listStore(
    (state) => ({
      listState: state.listState,
      ...
      <Droppable droppableId={String(_id)}>
        {(provided) => (
          ...
          >
            {cards.map(({ cardName, _id: idCard }, i) => {
              return (
                <Draggable key={idCard} draggableId={String(idCard)} index={i}>
                  {(provided) => (
                    <li
                      key={i}
