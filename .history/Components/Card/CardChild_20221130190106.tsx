...
import { LabelContext } from '../../lib/context/DataContext/LabelContext';
...
const CardChild = ({ childTitle, labels }: CardChildren) => {
  ...
  const [labelsState, dispatchLabel] = useReducer(labelReducer, labels);
  ...
  return (
    <Fragment>
      <LabelContext.Provider value={{ labelsState, dispatchLabel }}>
        ...
        {isFeatureOpen && (
          <Portal>
            <CardFeaturesModal labels={labelsState} />
          </Portal>
        )}
