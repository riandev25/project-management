...
const CardOptions = ({
  ...
  return (
    <div
      ref={ref}
      className={`${pos} z-20 -top-2 -left-2 absolute w-screen xs:w-[260px] rounded-sm bg-transparent text-gray-600`}
    >
      <div className='flex flex-col-reverse gap-2 xs:gap-0 xs:flex-row'>
        ...
        {delayedBtn && (
          <section
            className={`relative xs:absolute top-0 -right-24 bg-transparent transition delay-1000 w-fit grid-rows-1 xs:grid xs:grid-cols-1 xs:translate-x-2`}
          >
            {actions.map((item, i) => {
              return (
                <CardOptionsButton
