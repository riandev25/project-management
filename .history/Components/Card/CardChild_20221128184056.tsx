...
import { Children, Labels } from '../../interfaces/data';

const CardChild = ({ childTitle, labels }: Children) => {
      ...
      >
        <div className='flex flex-col w-full gap-2 shadow-sm'>
          <section className='grid grid-cols-5 gap-0.5'>
            {labels.map(({ id, iconColor }: Labels) => {
              return (
                <li key={id}>
                  <span className={`h-2 ${iconColor} rounded-md`}></span>
                </li>
              );
            })}
          </section>
