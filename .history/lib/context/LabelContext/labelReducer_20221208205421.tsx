...
const dataReducer = (labelState: ILabels[], action: labelAction): ILabels[] => {
  switch (action.type) {
    ...
    case 'UPDATE_LABEL':
      const updateLabel = () => {
        const { id, bgColor, iconColor, bgColorStrip, bgColorHover, name } =
          action.payload;
        const newState = labelState.map((label) => {
          return label.id === id
            ? {
    ...
    case 'SEARCH_LABEL':
      const searchLabel = () => {
        const filteredArray = labelState.filter((label) => {
          if (action.payload.value === '') return labelState;
          return label.name
            ?.toLowerCase()
            .includes(action.payload.value.toLowerCase());
        });
        action.payload.setArray(filteredArray);
