  ...
  switch (action.type) {
    ...
    case 'DELETE_CHECKLIST':
      const deleteChecklist = () => {
        ...
        const newChecklistItems = filteredData.map((filtered, i) => {
          return { ...filtered, id: i };
        });
      ...
      const deleteChecklistItem = () => {
        ...
        const newChecklist = checklistData.map((checklist) => {
          if (checklist.id === idCheckArray && idBtn === 'option-btn') {
            const filteredChecklistItems = checklist.checklistItems.filter(
              (checklistItems) => checklistItems.id !== idCheck
            );
            const newChecklistItems = filteredChecklistItems.map(
              (filtered, i) => {
                return { ...filtered, id: i };
              }
            );
