...
const recursion = (start: number, finished: number): any => {
  let sum = 2;
  if (start === finished) return sum;
  sum = start + sum;
  const index = start + 1;
  // const sum = value + 1;
  // if (sum > 19) return sum;
  return recursion(index, finished);
};

const answer = recursion(1, 19);
...
const HeaderModal = ({
  ...
  return (
    <header className='relative flex justify-center items-center py-1.5 w-full border-b border-gray-800'>
      {leftBtn && (
        <button
          data-id={id}
          type='button'
      ...
      <button
        ...
        type='button'
        className='absolute top-0 right-0 rounded-full px-1.5 hover:bg-gray-200'
